...
def run_get_mask(args):
    ...
    net_gcn = net_gin.GINNet(args['embedding_dim'], g, args['device'], args['spar_wei'], args['spar_adj'], coef=args['coef'])
    ...
    for epoch in range(args['total_epoch']):        
        ...
        if args['spar_wei']:
            for layer in net_gcn.modules():
                if isinstance(layer, layer_gin.MaskedLinear):
                    wei_loss += args['e2'] * \
        ...
        with torch.no_grad():
            ...
            if acc_val > best_val_acc['val_acc'] and aspar_here > args['target_adj_spar']:
...
def run_fix_mask(args, edge_masks, wei_masks, rewind_weight=None):
    ...
    net_gcn = net_gin.GINNet(embedding_dim=args['embedding_dim'], graph=g, device=device, spar_wei=args['spar_wei'], spar_adj=False, mode="retain")
    net_gcn = net_gcn.to(device)
...
if __name__ == "__main__":
    ...
    print(edge_masks)
    # print(edge_masks.requires_grad)
    print("="*50)

    high = 0
    if args['continuous']:
        for mask in edge_masks:
            ret = run_fix_mask(args, mask, [], rewind_weight=None)
            if ret > high:
                high = ret
            print("="*10)
            print(high)
            print("="*10)
    print(args['target_adj_spar'], high)
